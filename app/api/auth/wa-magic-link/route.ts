...
export async function POST(req: Request): Promise<Response> {
  ...
  // WA path: kirim OTP 6-digit via WA, trainer input di form login.
  // generateLink return email_otp yang bisa diverifikasi client-side via verifyOtp.
  if (waReady) {
    try {
      ...
      const text = `Kode masuk TrainingScout kamu:\n\n*${otp}*\n\nMasukkan kode ini di halaman login. Berlaku ~1 jam, sekali pakai.`;
      try {
        await sendText(jid, text);
        console.log(
          `[auth/wa-magic-link] sent_via=wa otp email=${maskEmail(email)} jid=${maskJid(jid)}`,
        );
        return NextResponse.json({
          ok: true,
          channel: "wa",
